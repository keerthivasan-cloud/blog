...
import { useContent, DEFAULT_CATEGORIES } from '../../context/ContentContext';

const Categories = () => {
  const { categories, addCategory, removeCategory, articles } = useContent();
  const [newCat, setNewCat] = useState('');

  const handleAdd = (e) => {
    e.preventDefault();
    if (!newCat.trim()) return;
    if (categories.map(c => c.toLowerCase()).includes(newCat.trim().toLowerCase())) {
  ...
  const handleDelete = (name) => {
    if (DEFAULT_CATEGORIES.includes(name)) {
      toast.error("Can't delete a default category");
      return;
    }
    removeCategory(name);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Organize your content into topics. New categories appear instantly in AI Generator, Manual Write, and Markdown Upload.</p>
          </div>
        </div>
          ...
          <div className="md:col-span-1">
            <form onSubmit={handleAdd} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 bg-primary hover:bg-primary/90 text-white rounded-lg font-medium shadow-sm transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                <Plus size={16} /> Create
              </button>
            </form>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
              <table className="w-full text-left border-collapse text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {categories.map((name) => (
                    <tr key={name} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <Tag size={16} className="text-slate-400" />
                          <span className="font-medium text-slate-900 dark:text-white">{name}</span>
                          {DEFAULT_CATEGORIES.includes(name) && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-400 font-medium">default</span>
                          )}
                        </div>
                      </td>
